...
function MyPage() {
  ...
  const handleInput1 = (e) => {
    setToInfo(e.target.value);
  };
  const handleInput2 = (e) => {
    setNumInfo(Number(e.target.value));
  };
      ...
      return (
        ...
        >
          <Form.Item
            label={selInput(val)}
            name={selInput(val)}
            rules={[{ required: true, message: `${selInput(val)}를 입력하세요.` }]}
          >
  ...
  const handleTransfer = () => {
    ...
    if (toInfo === '' || numInfo === '' || numInfo === NaN) {
      ...
      if (transType === '토큰') {
        ...
        axios.post('http://localhost:4000/token/transfer_20', req_body, req_header).then((res) => {
          console.log(res);
          if (res.status === 200) {
            console.log(res);
      ...
      if (transType === 'NFT') {
        ...
        console.log('req body', req_body);
        axios.post('http://localhost:4000/token/transfer_721', req_body, req_header).then((res) => {
          if (res.status === 200) {
            console.log(res);
            window.alert('NFT 전송 완료!');
            window.location.reload();
          } else {
            window.alert('NFT 전송 실패!');
            window.location.reload();
          }
        });
      }
          ...
          <ElWrapper>
            <>
              <div style={{ paddingBottom: '20px' }}>
                <Button shape="round" onClick={tokenClick}>
                  {'토큰 전송'}
                </Button>
                &ensp;&ensp;&ensp;&ensp;
                <Button shape="round" onClick={nftClick}>
                  {'NFT 전송'}
                </Button>
              </div>
